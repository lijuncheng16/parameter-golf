...
def zeropower_via_newtonschulz5_batched(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    ...
    G: (batch, m, n) — each matrix is orthogonalized independently via bmm.
    Grouping same-shape matrices from different layers into one call reduces
    kernel launches and improves GPU utilization vs. per-matrix NS.
    """
...
class Muon(torch.optim.Optimizer):
    ...
    DDP's backward already all_reduces gradients, so all ranks hold identical
    .grad tensors. NS is deterministic, so every rank computes the same update
    independently — no all_reduce or all_gather needed after the backward pass.
    Same-shape matrices across layers are batched into a single NS call.
    """
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            # Phase 1: apply momentum to all params, collect effective gradients.
            # No rank filtering — DDP pre-synced gradients so all ranks compute
            # identical NS updates and apply them locally without communication.
            items = []  # (param, eff_grad_bf16)
            for p in params:
                if p.grad is None:
                    continue
                g = p.grad
            ...
            # Phase 2: batch same-shape matrices together for a single NS call each.
            ...
            for shape, bucket in shape_buckets.items():
                G_batch = torch.stack([g for _, g in bucket])           # (B, m, n)
                # MuonEq-R: row-normalize before NS so each row has equal influence
                # on the orthogonalization. Equalizes singular value spread → better NS convergence.
