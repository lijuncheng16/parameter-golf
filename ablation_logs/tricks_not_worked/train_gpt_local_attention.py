...
class Hyperparameters():
    ...
    local_attention_heads = int(os.environ.get('LOCAL_ATTENTION_HEADS', 0))
    local_attention_window = int(os.environ.get('LOCAL_ATTENTION_WINDOW', 128))
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
                 ...
                 local_attention_heads: int = 0, local_attention_window: int = 128):
        super().__init__()
        if dim % num_heads != 0:
        ...
        if not 0 <= local_attention_heads <= num_heads:
            raise ValueError(
                f"local_attention_heads must be between 0 and num_heads; "
                f"got {local_attention_heads} for num_heads={num_heads}"
            )
        if local_attention_heads > 0 and local_attention_window <= 0:
            raise ValueError(
                f"local_attention_window must be positive when local heads are enabled; "
                f"got {local_attention_window}"
            )
        ...
        self.use_xsa = False
        self._local_mask_cache: dict[tuple[int, torch.device, torch.dtype], Tensor] = {}

    def _get_local_mask(self, seqlen: int, device: torch.device, dtype: torch.dtype) -> Tensor:
        key = (seqlen, device, dtype)
        mask = self._local_mask_cache.get(key)
        if mask is None:
            q_idx = torch.arange(seqlen, device=device)
            k_idx = torch.arange(seqlen, device=device)
            allowed = (q_idx[:, None] >= k_idx[None, :]) & (
                (q_idx[:, None] - k_idx[None, :]) < self.local_attention_window
            )
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self.local_attention_heads == 0:
            ...
            k_full = k.repeat_interleave(group_size, dim=2)
            v_full = v.repeat_interleave(group_size, dim=2)
            q_t = q.transpose(1, 2)
            k_t = k_full.transpose(1, 2)
            v_t = v_full.transpose(1, 2)
            global_heads = self.num_heads - self.local_attention_heads
            outputs: list[Tensor] = []
            if global_heads > 0:
                outputs.append(
            ...
            outputs.append(
                F.scaled_dot_product_attention(
                    ...
                    k_t[:, global_heads:],
                    v_t[:, global_heads:],
                    attn_mask=self._get_local_mask(seqlen, x.device, q.dtype),
                    is_causal=False,
                )
            )
            y = torch.cat(outputs, dim=1).transpose(1, 2)
        if self.use_xsa:
    ...
    def __init__(self, h: Hyperparameters):
        ...
        self.blocks = nn.ModuleList([
            Block(h.model_dim, h.num_heads, h.num_kv_heads, h.mlp_mult, h.rope_base,
                  ...
                  local_attention_heads=h.local_attention_heads,
                  local_attention_window=h.local_attention_window)
