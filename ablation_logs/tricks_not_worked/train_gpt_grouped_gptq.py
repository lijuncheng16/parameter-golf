...
class Hyperparameters():
    ...
    # Group size for GPTQ: one scale per group of columns (0 = per-row, no groups)
    gptq_group_size = int(os.environ.get('GPTQ_GROUP_SIZE', 64))
...
@torch.compile
def zeropower_via_newtonschulz5_batched(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
...
class Muon(torch.optim.Optimizer):
    """Muon optimizer with zero extra communication in DDP."""
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                items.append((p, g.bfloat16()))

            from collections import defaultdict
            shape_buckets: dict = defaultdict(list)
            for p, g in items:
                shape_buckets[g.shape].append((p, g))

            for shape, bucket in shape_buckets.items():
                G_batch = torch.stack([g for _, g in bucket])
                G_batch = G_batch / G_batch.norm(dim=-1, keepdim=True).clamp_min(1e-7)
                U_batch = zeropower_via_newtonschulz5_batched(G_batch, steps=backend_steps)
                scale = max(1, shape[0] / shape[1]) ** 0.5
                U_batch = U_batch * scale
                for (p, _), u in zip(bucket, U_batch.unbind(0)):
                    if wd > 0.0:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Per-row int6 quantization with clip-search. Scale shape: [rows]."""
    t32 = t.float()
    if t32.ndim == 2:
...
def quantize_int6_grouped(t: Tensor, clip_range: int = 31, group_size: int = 64) -> tuple[Tensor, Tensor]:
    """Per-group int6 quantization (no GPTQ). Scale shape: [rows, n_groups].
    Fallback used when Cholesky decomposition fails in GPTQ."""
    t32 = t.float()
    if t32.ndim != 2:
    ...
    for gi in range(n_groups):
        g_start = gi * group_size
        g_end = min(g_start + group_size, cols)
        W_g = t32[:, g_start:g_end]
        best_q, best_s, best_err = None, None, float('inf')
        for pct in [0.9990, 0.9999, 1.0]:
            row_clip = torch.quantile(W_g.abs(), pct, dim=1) if pct < 1.0 else W_g.abs().amax(dim=1)
            s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
            q = torch.clamp(torch.round(W_g / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            err = (W_g - q.float() * s.float()[:, None]).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q, s, err
        Q[:, g_start:g_end] = best_q
        scales[:, gi] = best_s
    return Q, scales
...
def gptq_quantize_weight(
    ...
    """GPTQ with group quantization and Hessian-weighted scale selection.

    When group_size > 0 and group_size < cols:
      - Per-group scales (shape [rows, n_groups]), no actorder.
      - Scale per group chosen by Hessian-weighted reconstruction error.
      - GPTQ error compensation runs across the full matrix as usual.
    Otherwise:
      - Per-row scale (shape [rows]), actorder enabled.
      - Scale search uses Hessian-weighted MSE instead of plain MSE.
    """
    W_orig = w.float().clone()
    rows, cols = W_orig.shape
    H = H.float().clone()

    # Zero out dead columns; compute damping from live columns only.
    dead = torch.diag(H) == 0
    H[dead, dead] = 1
    live_diag = H.diag()[~dead]
    damp = 0.01 * live_diag.mean() if live_diag.numel() > 0 else 0.01
    H.diagonal().add_(damp)
    hdiag = H.diag()  # per-column importance, used for H-weighted error

    use_groups = 0 < group_size < cols

    if not use_groups:
        # Per-row scale with actorder + Hessian-weighted scale selection.
        perm = torch.argsort(hdiag, descending=True)
        invperm = torch.argsort(perm)
        W_perm = W_orig[:, perm].clone()
        W_perm[:, dead[perm]] = 0
        H_perm = H[perm][:, perm]
        hdiag_perm = H_perm.diag()
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            # Hessian-weighted MSE: columns with high activation variance penalized more.
            mse = ((W_perm - recon).pow(2) * hdiag_perm[None, :]).mean().item()
            if mse < best_err:
                best_q, best_scale, best_err = Q, s, mse

        return best_q[:, invperm], best_scale

    # --- Group quantization path ---
    # No actorder: per-group scales already provide fine-grained adaptation.
    W_work = W_orig.clone()
    W_work[:, dead] = 0
    ...
    try:
        ...
        return quantize_int6_grouped(W_orig, clip_range, group_size)

    n_groups = (cols + group_size - 1) // group_size
    Q = torch.zeros(rows, cols, dtype=torch.int8)
    scales = torch.zeros(rows, n_groups, dtype=torch.float16)

    sf = None  # current group scale; set at each group boundary
    for i1 in range(0, cols, block_size):
        i2 = min(i1 + block_size, cols)
        W_block = W_work[:, i1:i2].clone()
        Hinv_block = Hinv[i1:i2, i1:i2]
        ...
        for j in range(i2 - i1):
            ...
            # At each group boundary, select scale minimising H-weighted error.
            # Scale is computed from original weights so it is not biased by
            # error compensation that has already been applied to W_work.
            if col_idx % group_size == 0:
                gi = col_idx // group_size
                g_end = min(col_idx + group_size, cols)
                W_g = W_orig[:, col_idx:g_end]
                hd_g = hdiag[col_idx:g_end]

                best_sf, best_err = None, float('inf')
                for pct in [0.9990, 0.9999, 1.0]:
                    row_clip = (torch.quantile(W_g.abs(), pct, dim=1)
                                if pct < 1.0 else W_g.abs().amax(dim=1))
                    sf_c = (row_clip / clip_range).clamp_min(1.0 / clip_range).float()
                    q_c = torch.clamp(torch.round(W_g / sf_c[:, None]), -clip_range, clip_range).float()
                    err = ((W_g - q_c * sf_c[:, None]).pow(2) * hd_g[None, :]).mean().item()
                    if err < best_err:
                        best_err, best_sf = err, sf_c
                sf = best_sf
                scales[:, gi] = sf.to(torch.float16)

            w_col = W_block[:, j]
            d = Hinv_block[j, j]
            q_col = torch.clamp(torch.round(w_col / sf), -clip_range, clip_range)
            Q[:, col_idx] = q_col.to(torch.int8)
            err_c = (w_col - q_col.float() * sf) / d
            Err[:, j] = err_c
            W_block[:, j:] -= err_c.unsqueeze(1) * Hinv_block[j, j:].unsqueeze(0)
...
def gptq_mixed_quantize_int6(
    ...
    """Mixed quantization: GPTQ (grouped) for layers with Hessians, fallback otherwise."""
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim == 2:
            use_groups = 0 < group_size < t.shape[1]
            if name in hessians:
                q, s = gptq_quantize_weight(t, hessians[name], group_size=group_size)
                gptq_count += 1
                method = "gptq_grouped" if use_groups else "gptq"
                meta[name] = {"type": "int6", "method": method, "group_size": group_size}
            else:
                if use_groups:
                    q, s = quantize_int6_grouped(t, group_size=group_size)
                    meta[name] = {"type": "int6", "method": "grouped_clip_search", "group_size": group_size}
                else:
    ...
    log(f"GPTQ quantization: {gptq_count} layers with GPTQ, {fallback_count} fallback")
    return result, meta
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            meta[name] = "passthrough_ctrl"
            continue
        if cat in int6_cats and t.ndim == 2 and 0 < group_size < t.shape[1]:
            q, s = quantize_int6_grouped(t, group_size=group_size)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "method": "grouped", "group_size": group_size}
        elif cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim == 2:
            # Group quantization: s is [rows, n_groups], expand to [rows, cols].
            cols = q.shape[1]
            n_groups = s.shape[1]
            group_size = (cols + n_groups - 1) // n_groups
            s_expanded = s.float().repeat_interleave(group_size, dim=1)[:, :cols]
            out[name] = (q.float() * s_expanded).to(orig_dtype)
        elif s.ndim > 0:
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> int:
    ...
    if h.gptq_enabled:
        ...
        log(f"GPTQ:group_size={h.gptq_group_size}")
        ...
        quant_result, quant_meta = mixed_quantize_int6(
            sd_cpu, {"mlp", "attn"}, group_size=h.gptq_group_size)
